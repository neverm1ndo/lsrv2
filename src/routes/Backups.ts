...
router.get('/list', (req: any, res: any) => {
	...
	void (async () => {
		try {
			const backups = await BACKUP.find({}).sort({ unix: -1 }).exec();
	...
	void (async () => {
		try {
			if (!req.params.hash) {
				...
				return void res.status(CONFLICT).send(CommonErrors[ErrorCode.PARAM_MISSING]);
			}
...
router.get('/restore/:hash', (req: any, res: Response) => {
	...
	void (async () => {
		try {
			...
			return void res.status(INTERNAL_SERVER_ERROR).send(err.message);
		}
	})();
});
...
router.delete('/backup/:hash', (req: any, res: Response) => {
	...
	void (async () => {
		try {
			...
			return void res.status(INTERNAL_SERVER_ERROR).send(message);
		}
	})();
});
