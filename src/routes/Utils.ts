...
import { mkdir, move, rmdir } from 'fs-extra';
 ...
 */
router.delete('/delete-file', json(), (req: Request, res: Response) => {
	if (!req.query.path) {
	...
	void (async () => {
		...
		try {
			...
			return void res.status(INTERNAL_SERVER_ERROR).end('Backuper error: ' + err.message);
		}
	})();
});
 ...
 */
router.post('/mkdir', json(), (req: Request<any, any, { path?: string }>, res: Response) => {
