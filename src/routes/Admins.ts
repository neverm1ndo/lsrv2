...
const { GET_ADMIN_LIST, CHANGE_MAIN_GROUP, CHANGE_SECONDARY_GROUP, ADD_SECONDARY_GROUP, REMOVE_SECONDARY_GROUP } =
	SQLQueries;
...
router.post('/add-group', Guards.developerGuard as Handler, json(), corsOpt, (req: any, res: any) => {
	logger.log(
		LOGGER_PREFIX,
		'[POST]',
		'ADD_SECONDARY_GROUP',
		...
		req.user.username,
		Workgroup[req.user.main_group],
		`{${req.body.id} : ${Workgroup[req.body.group]}}`
	);
	...
	void (async () => {
		try {
			...
			await MSQLPool.promise().query(ADD_SECONDARY_GROUP, [req.body.id, req.body.group]);
...
router.delete('/delete-group', Guards.developerGuard as Handler, json(), corsOpt, (req: any, res: any) => {
	logger.log(
		LOGGER_PREFIX,
		'[POST]',
		'DELETE_SECONDARY_GROUP',
		`(${req.connection.remoteAddress})`,
		req.user.username,
		Workgroup[req.user.main_group],
		`{${req.body.id} : ${Workgroup[req.body.group]}}`
	...
	void (async () => {
		try {
			...
			await MSQLPool.promise().query(REMOVE_SECONDARY_GROUP, [req.body.id, req.body.group]);
...
router.patch('/change-group', Guards.developerGuard as Handler, json(), corsOpt, (req: any, res: any) => {
	logger.log(
		LOGGER_PREFIX,
		'[PATCH]',
		'CHANGE_ADMIN_GROUP',
		...
		req.user.username,
		Workgroup[req.user.main_group],
		`{${req.body.username} : ${Workgroup[req.body.group]}}`
	);
	...
	MSQLPool.promise()
		...
		.catch((err: any): void => {
			...
			logger.err(
				...
				`[${req.connection.remoteAddress}]`,
				req.user.username,
				`{${req.body.username} : ${Workgroup[req.body.group]}} FAIL`,
				err.code || 0
			);
...
router.patch('/change-secondary-group', Guards.developerGuard as Handler, json(), (req: any, res: any) => {
	logger.log(
		LOGGER_PREFIX,
		'[PATCH]',
		'CHANGE_ADMIN_SECONDARY_GROUP',
		`(${req.connection.remoteAddress})`,
		req.user.username,
		Workgroup[req.user.main_group],
		`{${req.body.username} : ${Workgroup[req.body.group]}}`
	...
	if (!req.body.id && !req.body.group) {
		logger.err(
			LOGGER_PREFIX,
			'[PATCH]',
			'CHANGE_ADMIN_SECONDARY_GROUP_FAIL',
			`(${req.connection.remoteAddress})`,
			req.user.username,
			Workgroup[req.user.main_group],
			`{${req.body.username} : ${Workgroup[req.body.group]}}`,
	...
	MSQLPool.promise()
		...
		.then((): void => {
			logger.log(
				LOGGER_PREFIX,
				'[PATCH]',
				'CHANGE_ADMIN_SECONDARY_GROUP_SUCCESS',
				`(${req.connection.remoteAddress})`,
				req.user.username,
				Workgroup[req.user.main_group],
				`{${req.body.username} : ${Workgroup[req.body.group]}}`
