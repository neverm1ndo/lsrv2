...
router.get('/config-files-tree', (req: Request, res: Response) => {
	...
	void (async () => {
		...
		if (!root) {
			...
			return void res.status(INTERNAL_SERVER_ERROR).send('Cant read file tree');
		}
...
router.get('/file-info', (req: Request, res: Response) => {
	if (!req.query.path) {
		return void res.status(CONFLICT).send('Empty path param');
	}
...
router.post('/save-file', upcfg.fields([{ name: 'file', maxCount: 1 }]), (req: Request, res: Response) => {
