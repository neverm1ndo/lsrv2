...
const { MOVED_PERMANENTLY, INTERNAL_SERVER_ERROR, CONFLICT, NOT_IMPLEMENTED, OK } = StatusCodes;
...
async function sendDBRequest(request: string, params?: any[]) {
	return MSQLPool.promise().query(request, params);
}
	...
	void (async () => {
		try {
			logger.log(
				...
				req.user.username,
				Workgroup[req.user!.main_group],
				`AS:${req.query.as} SS:${req.query.ss}`
			);
