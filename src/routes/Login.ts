...
import { Handler, json, NextFunction, Router } from 'express';
	...
	passport.authenticate('local', (err, user) => {
		try {
			logger.log(
				...
				Workgroup[req.user?.main_group] || 'no_group'
			);
			...
			if (err || !user) {
				logger.err(
					...
					`ERROR_OCCURED::${err?.message || 'no_username'}::`
				);
			...
			req.login(user, { session: true }, (error: any) => {
				if (error) {
					logger.err(
						...
						req.user?.username,
						Workgroup[req.user?.main_group],
						`ERROR_OCCURED::${error.message || 'no_username'}::`
					);
					return next(error);
				}
...
const reqLogout = (req: any, _res: any, next: NextFunction) => {
	req.logout((err: any) => {
...
router.post('/logout', cors, passport.authenticate('jwt') as Handler, reqLogout, (req: any, res: any) => {
	...
	logger.err(
		...
		`(${req.socket.remoteAddress})`,
		req.user?.username,
		Workgroup[req.user?.main_group as number]
	);
