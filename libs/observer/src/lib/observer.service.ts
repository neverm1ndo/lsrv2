...
export const OBSERVER_OPTIONS = 'OBSERVER_OPTIONS';

const EOL_BUFFER: Buffer = Buffer.from(EOL);

@Injectable()
export class ObserverService implements OnModuleDestroy {
	...
	constructor(@Optional() @Inject(OBSERVER_OPTIONS) private options: ObserverOptions) {}

	/**
	 * You can configure options dynamically if not provided in the constructor.
	 */
	setOptions(options: ObserverOptions) {
		this.options = options;
	}

	observe(): Stream {
		if (!this.options || !this.options.path) {
			throw new Error('ObserverOptions path is not configured.');
		}

		this.fsWatcher = watch(this.options.path, this.watcherOptions);
		...
		this.fsWatcher.on('error', (err) => logger.error(err, 'Observer error'));
